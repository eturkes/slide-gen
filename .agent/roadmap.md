...
## Milestone ledger
- **M1 CLI foundation** — discovery + toggle state (no codex, no render). STATUS: **IMPLEMENTED** (units DONE, unreviewed).
...
Locked design:
- **Async-from-start**: `async fn main` lands M1.2 at the first `@fs.kind` await — an empty `async fn main` warns `unused_async` (blocks clean check), so M1.1 main is sync `fn main`. `@fs.kind` (lstat) + `@process` are async-only; M1.2 = flip `fn`→`async fn` + add the `moonbitlang/async` (runtime) + `moonbitlang/async/fs` (`@fs`) imports to `cmd/main/moon.pkg` + the first await — cheap, not a retrofit.
...
- **Discovery** (implemented M1.2): `@fs.readdir(parent, include_hidden=true)` (bare names) → `@fs.kind(parent/name)` on the FULL path (a bare name resolves vs cwd, not the scanned parent). `kind` FOLLOWS symlinks by default + raises on an unresolvable target → ONE follow-stat classifies candidate (real dir or symlink→dir = `Directory`) vs reject; the planned `follow_symlink=false` lstat + 2nd stat proved redundant. Any `kind` error (broken link / racing unlink / unreadable) → skip, so one bad sibling never aborts the scan. Skip dotfiles + the `exclude` name; no recursion. Sort by a codepoint comparator (`String`'s default `Compare` is shortlex → length-ordered). `parent` for `list` = cwd's parent (`@env.current_dir` → `split_path`) → M1.2 contract = siblings of CWD (the repo is the run-from home); run-from-anywhere repo-root anchoring → M1.3. Verified set (post self-exclude): 33 dirs + 5 symlinks = 38 (28 git); raw `~/Projects/` = 34 dirs + 5 symlinks = 39.
- **UX**: subcommands `list | enable <name> | disable <name> | toggle <name> | status` via `@argparse`; reject `<name>` ∉ candidates.
- **State**: `Set[String]` of enabled names → newline-delimited, sorted; atomic write = unique temp in the SAME dir → `rename` over `enabled.txt` (last-writer-wins; single-user → no lock; fsync if the async API exposes it; gitignore the temp-name pattern alongside `enabled.txt` once M1.3 fixes the naming — a crash leaves the temp visible otherwise); missing file = empty set; gitignored repo-root `enabled.txt`; default-off. (`@json` struct is the upgrade path if state gains structure.)
...
- M1.3 codex-review deferrals (revisit in M1 review): (a) repo-root anchor accepts ANY ancestor `moon.mod` → running the binary from inside an unrelated MoonBit project mis-anchors (writes `enabled.txt` + discovers siblings there); validate module name / a slide-gen sentinel? (b) `parse_state` never validates on read (total / never-trim is locked) → an externally-tampered unrepresentable stale line is untypeable for `disable`; a `prune` path would clear it. (c) Fixed predictable temp = single-user tradeoff (concurrent writers corrupt; a pre-planted `enabled.txt.tmp` symlink redirects the write) — documented + accepted under single-user, not remotely exploitable.

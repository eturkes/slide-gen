...
- Install: `curl -fsSL https://cli.moonbitlang.com/install/unix.sh | bash` → `~/.moon/bin` (moon, moonc, moonfmt…). Pin via the installer's version arg / `MOONBIT_INSTALL_VERSION` + record `moon version` (moonup = 3rd-party, not installed → verify before relying on it). Native needs `build-essential` (cc); supports x86_64 + aarch64.
- Core built-ins (no dep): `@argparse` (clap-like subcommands), `@quickcheck` (Arbitrary + `samples` generators for sample-driven tests; not a full property runner/shrinker, no fuzzer), `@json` (derive ToJson/FromJson), `@env.args()`. No TOML lib → newline state file.
...
- Gotchas: `"is-main"` (hyphen, not `is_main`); native binary lands under the build tree (`target/` or `_build/`, path unverified across versions) → locate empirically, prefer `moon run`, never hardcode; for native runs use `moon run` (verify `moonrun`'s scope — historically wasm/js only — after install if needed); async + `@argparse` APIs churn → use `inspect` snapshots to catch drift on upgrade; `x/fs.is_dir` follows symlinks + lacks lstat → use async `@fs.kind` on the FULL path `parent/name` (a bare readdir name resolves against cwd). Serena has no MoonBit LSP → grep + `moon check`/`mooninfo` for nav.
